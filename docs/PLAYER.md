...
at the corner rect while the `mini` flag stayed wherever it was last set —
undock a full-size player and you got a corner-sized video wearing
...
**There is no minimise button.** The corner is where navigation puts the player,
not a mode the viewer picks, so `undock()` is the only way in. The button that
used to sit top-left of the docked chrome was gated on PiP support and did not
use PiP — it called `setMini(true)`, our own corner window — so a device without
PiP hid a control that would have worked perfectly. Leaving the page is already
the gesture that means "keep playing while I go elsewhere", and the host reports
...
So when the user leaves page A with the video playing, navigates to page B and
presses expand,
...
| **`qualityIcon`**      | <code>'default' \| 'downloaded'</code>                              | `downloaded` adds a download-done tick after the quality button's label, for a video playing from a copy on the device; `default` removes it. Absent keeps the current state.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| **`detail`**   | <code>string</code>  | Secondary text after the label, in the sheet's muted colour — e.g. `Downloaded` on the row that plays from a copy on the device.              |
| **`selected`** | <code>boolean</code> | The row that is playing now, ticked. When no row sets it, the player ticks the row whose id or label matches the button's label.              |
